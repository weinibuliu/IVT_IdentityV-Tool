...
def act_name_list() -> list[str]:
    return (
        ["Fight"]
        + ["Thumb_Ups", "Move", "Vision_Move", "Hide_Mixed_Move_Jump"]
        + ["OS_Round"]
    )
...
class Fight(CustomAction):
    def run(self, context: Context, argv: CustomAction.RunArg) -> bool:
        ...
        def fight_main(character: str):
            ...
            check_fight_statu = context.run_recognition(
                "fight_赛后_继续_仅识别",
            ...
            match character:
                ...
                case _:
                    raise ValueError(
                        f"Class Error:{__class__.__name__},please contact to the developers."
                    )
        ...
        def hide_main():
            ...
            while task_statu is None and task_statu != "继续":
                ...
                task_statu = context.run_recognition(
                    "fight_赛后_继续_仅识别",
                    context.tasker.controller.post_screencap().wait().get(),
                )
            ...
            if model == "匹配模式" or model == "排位模式":
                ...
                if model == "排位模式":
                    context.run_pipeline(
                        "确认禁用", pipeline_override={"确认禁用": {"timeout": 3000}}
                    )
                context.override_pipeline(
                    {"fight_选择角色": {"template": f"characters//{character}.png"}}
                )
        ...
        def main(
            desktop_notice: bool = desktop_notice,
            email_notice: bool = email_notice,
            model_list: list = model_list,
            model_list_random: bool = model_list_random,
            character_list: list = character_list,
            character_list_random: bool = character_list_random,
            thumbs_up: bool = thumbs_up,
            reputation_limit: int = reputation_limit,
            up_weekly: bool = up_weekly,
            limit_time: bool | int | float = limit_time,
            limit_times: bool | int = limit_times,
            check_reputation_rate: int = check_reputation_rate,
            check_weely_rate: int = check_weely_rate,
        ):

            def list_ramdon(
                m_list_random: bool = model_list_random,
                c_list_random: bool = character_list_random,
            ):  # 模式、角色队列乱序
            ...
            while weekly and limit_time and limit_times and reputation_limit:
                ...
                for character in character_list:
                    ...
                    if (
                        weekly == False
                        or limit_time == False
                        or limit_times == False
                        or reputation_limit == False
                    ):
                        break
                    ...
                    for model in model_list:
                        ...
                        if thumbs_up == False:
                            context.override_pipeline(
                                {"fight_赛后_继续": {"next": ["fight_赛后_返回大厅"]}}
                            )
                        else:
                            context.override_pipeline(
                                {
                        ...
                        if (
                            weekly_flag != False
                            and fight_times_weekly == check_weely_rate
                        ):
                            fight_times_weekly = int(0)
                            context.run_pipeline("fight_检测周上限_打开推理之径")
                            sleep(3.5)
                            weekly_detail = context.run_recognition(
                                "fight_检测周上限",
                                context.tasker.controller.post_screencap().wait().get(),
                            )
                            if (
                                weekly_detail is not None
                                and int(weekly_detail.best_result.text) == 42000
                                or int(weekly_detail.best_result.text) == 50400
                            ):
                                weekly = bool(False)
                                break
                        ...
                        if (
                            ...
                            and fight_times_reputation == check_reputation_rate
                        ):
                            fight_times_reputation = int(0)
                            context.run_pipeline("fight_检测人品值_打开个人名片")
                            sleep(3.5)
                            reputation_detail = context.run_recognition(
                                "fight_检测人品值",
                                context.tasker.controller.post_screencap().wait().get(),
                            )
                            if (
                                reputation_detail is not None
                                and int(reputation_detail.best_result.text)
                                <= reputation_limit
                            ):
